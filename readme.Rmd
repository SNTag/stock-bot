...
Keep note that the file "/stock.csv" is an example file. Please replace with your own. Details on the csv file can be found under "Technical Details > Tracking companies and the investment plan they are under"

The main script is "/stock-bot.R". If you have not provided key details, it will ask you for them.

To add the stock-bot to crontab, to run everyday at 11:00AM, run '/setup.R'.
...
Using a basic csv sheet (/input/stock.csv), each company and the plan they are under are read. Informing the program what investment plan they are under is important only to the notifications you receive.
...
The column for investment.plan relates to the portfolio the are in. This is for a future feature with better notifications. It is currently unused.
